...
impl Archiver {
    pub async fn get_boards_with_full_images(&self) -> Result<HashSet<String>, ()> {
        let boards = self.db_client.get_all_boards().await.map_err(|e| {
    ...
    pub async fn image_cycle(&self) -> Result<(), ()> {
        ...
        loop {
            let mut jobs = self.db_client.get_image_jobs(250).await.map_err(|e| {
            ...
            while let Some(job) = jobs.pop() {
                ...
                running_jobs.insert(
                    job.id,
                    self.dispatch_archive_image(
                        tx.clone(),
                        full_image_boards.contains(&job.board),
                        job,
                    ),
                );
                ...
                if let Some(job_id) = rx.recv().await {
                    ...
                    // All jobs are terminated or dead
                    running_jobs.clear();
                }
    ...
    pub fn dispatch_archive_image(
        &self,
        tx: tokio::sync::mpsc::Sender<i64>,
        need_full_image: bool,
        job: ImageJob,
        ...
        tokio::task::spawn(async move {
            ...
            let s = Instant::now();
            AssertUnwindSafe(c.archive_image(&job.clone(), need_full_image))
                .catch_unwind()
                .await
                .ok();
    ...
    pub async fn archive_image(&self, job: &ImageJob, need_full_image: bool) -> Result<(), ()> {
        ...
        if thumbnail_sha256.is_empty() {
            thumbnail_sha256 = self
                .http_client
                .download_file_checksum(&job.thumbnail_url, &".jpg".to_string(), true)
                .await?;
            counter!("thumbnails_fetched", 1);
            info!("Processed thumbnail for /{}/{}", job.board, job.no);
            self.db_client
                .set_post_files(&job.board, job.no, &file_sha256, &thumbnail_sha256)
                .await
                .map_err(|e| {
                    error!(
                        "Failed to update file for post: /{}/{}: {}",
                        job.board, job.no, e
                    );
                })?;
        }
        ...
        if file_sha256.is_empty() && need_full_image {
            file_sha256 = self
                .http_client
                .download_file_checksum(&job.url, &job.ext, false)
                .await?;
        ...
        self.db_client.delete_image_job(job.id).await.map_err(|e| {
            error!("Failed to delete file job {} from backlog: {}", job.id, e);
        })?;
        Ok(())
    }
